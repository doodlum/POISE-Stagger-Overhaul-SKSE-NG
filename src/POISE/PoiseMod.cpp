...
void Loki::PoiseMod::InstallMagicEventSink()
{
	...
	if (sourceHolder) {
		sourceHolder->AddEventSink(PoiseMagicDamage::GetSingleton());
	}
}
	...
	if (!a_event->target || !a_event->cause) {
		...
		if (auto projectile = RE::TESForm::LookupByID(a_event->projectile)->As<RE::Projectile>(); projectile) {
			if (auto actor = a_event->target.get()->As<RE::Actor>(); actor) {
				...
				if (auto effect = projectile->GetProjectileRuntimeData().spell->avEffectSetting; effect) {
				...
				actor->GetActorRuntimeData().pad0EC -= (int)a_result;
				if (actor->GetActorRuntimeData().pad0EC > 100000) {
					actor->GetActorRuntimeData().pad0EC = (uint32_t)0.00f;
				}

				if (ptr->ConsoleInfoDump)
					RE::ConsoleLog::GetSingleton()->Print("current poise health -> %f", actor->GetActorRuntimeData().pad0EC);
				...
				if (actor->GetHandle() == Form->GetHandle()) {
					stagDir = 0.0f;
				}  // 0 when self-hit
				...
				if ((float)actor->GetActorRuntimeData().pad0EC <= 0.00f) {
					actor->SetGraphVariableFloat(ptr->staggerDire, stagDir);   // set direction
					actor->GetActorRuntimeData().pad0EC = (uint32_t)maxPoise;  // remember earlier when we calculated max poise health?
					...
					if (actor->HasKeyword(ptr->kCreature) || actor->HasKeyword(ptr->kDwarven)) {  // if creature, use normal beh
						...
						if (projectile->GetProjectileRuntimeData().explosion) {
				...
				} else if ((float)actor->GetActorRuntimeData().pad0EC <= threshhold0 || (float)actor->GetActorRuntimeData().pad0EC <= 2.00f) {
					actor->SetGraphVariableFloat(ptr->staggerDire, stagDir);                      // set direction
					if (actor->HasKeyword(ptr->kCreature) || actor->HasKeyword(ptr->kDwarven)) {  // if creature, use normal beh
						actor->SetGraphVariableFloat(ptr->staggerMagn, 0.75f);
						actor->NotifyAnimationGraph(ptr->ae_Stagger);
					} else {
						if (projectile->GetProjectileRuntimeData().explosion) {  // check if explosion, dragon, giant attack or dwarven
							if (stagDir > 0.25f && stagDir < 0.75f) {
				...
				} else if ((float)actor->GetActorRuntimeData().pad0EC <= threshhold1 || (float)actor->GetActorRuntimeData().pad0EC <= 5.00f) {
					actor->SetGraphVariableFloat(ptr->staggerDire, stagDir);  // set direction
				...
				} else if ((float)actor->GetActorRuntimeData().pad0EC <= threshhold2 || (float)actor->GetActorRuntimeData().pad0EC <= 10.00f) {
					actor->SetGraphVariableFloat(ptr->staggerDire, stagDir);  // set direction
					if (actor->HasKeyword(ptr->kCreature) || actor->HasKeyword(ptr->kDwarven)) {
...
void Loki::PoiseMod::ProcessHitEvent(RE::Actor* a_actor, RE::HitData& a_hitData)
{
	...
	a_actor->GetActorRuntimeData().pad0EC -= (int)dmg;  // there was some bullshit with integer underflow
	if (a_actor->GetActorRuntimeData().pad0EC > 100000)
		a_actor->GetActorRuntimeData().pad0EC = (uint32_t)0.00f;  // this fixed it...
	if (ptr->ConsoleInfoDump) {
		...
		RE::ConsoleLog::GetSingleton()->Print("Victim's Current Poise Health: %d", a_actor->GetActorRuntimeData().pad0EC);
	...
	if ((float)a_actor->GetActorRuntimeData().pad0EC <= 0.00f) {
		a_actor->SetGraphVariableFloat(ptr->staggerDire, stagDir);   // set direction
		a_actor->GetActorRuntimeData().pad0EC = (uint32_t)maxPoise;  // remember earlier when we calculated max poise health?
		if (TrueHUDControl::GetSingleton()->g_trueHUD) {
	...
	} else if ((float)a_actor->GetActorRuntimeData().pad0EC < threshhold0 || (float)a_actor->GetActorRuntimeData().pad0EC < 2.00f) {
		a_actor->SetGraphVariableFloat(ptr->staggerDire, stagDir);                        // set direction
		if (a_actor->HasKeyword(ptr->kCreature) || a_actor->HasKeyword(ptr->kDwarven)) {  // if creature, use normal beh
	...
	} else if ((float)a_actor->GetActorRuntimeData().pad0EC < threshhold1 || (float)a_actor->GetActorRuntimeData().pad0EC < 5.00f) {
		a_actor->SetGraphVariableFloat(ptr->staggerDire, stagDir);  // set direction
		if (a_actor->HasKeyword(ptr->kCreature) || a_actor->HasKeyword(ptr->kDwarven)) {
			a_actor->SetGraphVariableFloat(ptr->staggerMagn, 0.50f);
			a_actor->NotifyAnimationGraph(ptr->ae_Stagger);
		} else {
			if (a_hitData.flags == HitFlag::kExplosion || a_hitData.aggressor.get()->HasKeyword(ptr->kDragon) || a_hitData.aggressor.get()->HasKeyword(ptr->kGiant) || a_hitData.aggressor.get()->HasKeyword(ptr->kDwarven) || a_hitData.aggressor.get()->HasKeyword(ptr->kTroll) || a_hitData.aggressor.get()->GetRace()->formID == kLurker) {
				if (stagDir > 0.25f && stagDir < 0.75f) {
					str = ptr->poiseLargeFwd;
				} else {
					str = ptr->poiseLargeBwd;
	...
	} else if ((float)a_actor->GetActorRuntimeData().pad0EC < threshhold2 || (float)a_actor->GetActorRuntimeData().pad0EC < 8.00f) {
		a_actor->SetGraphVariableFloat(ptr->staggerDire, stagDir);  // set direction
		if (a_actor->HasKeyword(ptr->kCreature) || a_actor->HasKeyword(ptr->kDwarven)) {
